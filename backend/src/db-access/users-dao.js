...
const followUnfollowUser = async (userId, followUserId) => {
    ...
    const foundUser = await db.collection(usersCollectionName).find({_id: ObjectId(userId)}).toArray();
    ...
    return db.collection(usersCollectionName).findOneAndUpdate({_id: ObjectId(userId)}, {$set: {following: newFollowsArr}}, {returnDocument: "after"}) 
}

// Setzt die User bei .followedBy - von wem wir mir gefolgt
const followedUser = async (userId, followUserId) => {
    const db = await getDB();
    const foundFollowedUser = await db.collection(usersCollectionName).find({_id: ObjectId(followUserId)}).toArray();
