...
const findTweetsByKeyword = async (keyword) => {
	...
	const foundPosts = await db
		.collection(collectionName)
		.find({
			$and: [{ $contains: { content: keyword } }, { replyTo: null }],
		})
		.sort({ postedAt: -1 })
		.toArray();
	return foundPosts;
...
const findAllTweetsOfFollowedUsers = async (userId) => {
	...
	const repliesPackage = await db
		.collection(collectionName)
		.find({
			$and: [{ postedBy: { $in: followedUserIds } }, { replyTo: null }],
		})
		.sort({ postedAt: -1 })
		.toArray();
